...
function SocialLinks({ site }: { site: BlogSiteLinks }) {
  return (
    <div className="blog-social-links">
      ...
      <a href={site.hackmdUrl} target="_blank" rel="noreferrer">
        HackMD
      </a>
    </div>
  );
...
function getPageMeta({ site, post, currentPage = 1 }: { site: BlogSiteLinks; post?: BlogPost; currentPage?: number }) {
  if (!post) {
    return {
...
function BlogDocument({ site, posts, currentPage = 1, post, relatedPosts, previousPost, nextPost }: BlogDocumentProps) {
